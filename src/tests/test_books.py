from src.books.schemas import CreateABookModel

book_prefix = f"/api/v1/books"

def test_gat_all_books(test_client,fake_book_service,fake_session):
...
def test_create_book(test_client,fake_book_service,fake_session):
  ...
  book_create_data = CreateABookModel(**book_data)

  assert fake_book_service.get_all_books_called_once()
  assert fake_book_service.get_all_books_called_once_with(book_create_data,fake_session)


# def test_get_book_by_uid(test_client,fake_book_service,test_book,fake_session):
#   response = test_client.get(
#     url=f"{book_prefix}/{test_book.uid}"
#   )
